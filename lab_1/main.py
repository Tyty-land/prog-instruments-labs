...
from tools_dataframe_module import writer_csv, pandas_statistical_calculation, demonstration_of_results
...
def Det_p()->Tuple[str, str, str]:
    """
     A function that accepts command-line parameters, namely: -k = keyword to search for a photo (default = "None")-sd = path to the future folder with photos (default = "")
        -D = path and name to the .csv DataFrame (default = "DataFrame.csv")then collects these parameters into a tuple and returns this tuple
...
def main() -> None:
    ...
    Some functions are designed to output sequential changes to the data in the DataFrame,
