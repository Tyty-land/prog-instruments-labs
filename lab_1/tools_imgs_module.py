...
str) -> List[list]:
    ...
    data_imgs = [["Absolute Path:",
                  "Relative path:", "Height:", "Width:", "Color_depth:"]]
    for image in images:
...
def createAbsolut_dir(save_dir: str) -> str:
    ...
    or this folder is located on another disk or in another
    branch that is not adjacent to this one.
    If the folder should still be located in this directory
    from where the program is launched, then
    the path to the current directory where the program was
    launched is appended to the path
    :param save_dir: the path to the photo saving folder
    obtained by the command line parameter
    :return absolut_dir: The absolute path to the corresponding
    file or folder
    """
    if re.search(r"\w:/+", save_dir) is None \
            and re.search(r"\w:\\+", save_dir) is None:
